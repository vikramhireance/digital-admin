<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\Controller;
use App\Models\BlogCategory;
use Illuminate\Http\Request;
use Illuminate\Support\Str;
use Illuminate\Support\Facades\URL;
use Yajra\DataTables\DataTables;
use Illuminate\Support\Facades\File;

class BlogCategoryController extends Controller
{
    public function blog_category(Request $request)
    {
        if ($request->ajax()) {
            $data = BlogCategory::select('*');
            return Datatables::of($data)
                    ->addIndexColumn()
                    ->editColumn('image', function($slider){
                        return '<img src="'.URL::to('Upload/Blogs').'/'.$slider->image.'" width="60px">';
                    })
                    ->addColumn('action', function($data){
                        return view('admin.blog_category.action',compact('data'))->render();
                    })
                    ->rawColumns(['action','image'])
                    ->make(true);
        }
        return view('admin.blog_category.blog_index');
    }

    public function add(Request $request)
    {
        return view('admin.blog_category.blog_create');
    }
    public function store(Request $request)
    {
        
        $request->validate([
            'title' => 'required',
            'image' => 'required',
            'meta_title' => 'required',
            'meta_description' => 'required',
            'meta_keywords' => 'required',
        ]);

        $blog = new BlogCategory;
        $blog->title = $request->title;
        $blog->meta_title = $request->meta_title;
        $blog->image = $request->image;
        $blog->meta_description = $request->meta_description;
        $blog->meta_keywords = $request->meta_keywords;
        $blog->slug = Str::slug($request->title);

        if ($request->file('image')) {
           
            $file = $request->file('image');
            $filename =date('YmdHis') . "." . $file->getClientOriginalExtension();
            $file->move(public_path('Upload/Blogs'), $filename);
            $blog->image = $filename;
        }
        $blog->save();
        return redirect('/admin/blog-category')->with('success', "Serviice CAtegory Added successfully");
    }

    public function edit($id)
    {
        $data = BlogCategory::find($id);
        return view('admin.blog_category.blog_edit',compact('data'));
    }

    public function update(Request $request)
    {
        $data = BlogCategory::find($request->id);

        $request->validate([
            'title' => 'required',
            'meta_title' => 'required',
            'meta_description' => 'required',
            'meta_keywords' => 'required',
        ]);

        if ($request->file('image')) {
            if($data->image){
                $image_path = public_path('Upload/Portfolios/').$data->image;
                if(file_exists($image_path)){
                    File::delete($image_path);
                }
            }
            $file = $request->file('image');
            $filename =date('YmdHis') . "." . $file->getClientOriginalExtension();
            $file->move(public_path('Upload/Portfolios'), $filename);
            $input['image']  = $filename;
        }

        $input['title'] = $request->title;
        $input['meta_title'] = $request->meta_title;
        $input['meta_description'] = $request->meta_description;
        $input['meta_keywords'] = $request->meta_keywords;
        $input['slug'] = Str::slug($request->title);

        BlogCategory::find($request->id)->update($input);
        return redirect('/admin/blog-category')->with('success', "Serviice Category Updated successfully");
    }

    public function delete(Request $request)
    {
        $data =BlogCategory::where('id', $request->id)->first();
        if($data->image){
            $image_path = public_path('Upload/Portfolios/').$data->image;
            if(file_exists($image_path)){
                File::delete($image_path);
            }
        }

        BlogCategory::where('id', $request->id)->delete();
        return response()->json(['status'=>true,'message'=>'"Serviice Category deleted successfully"']); 
    }
}
